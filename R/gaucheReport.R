#' View solutions contained within gauche output
#' 
#' After running gauche() on data, this function provides a convenient way to view the solutions and also export them as separate
#' text files and images.  For detailed usage, please read the accompanying vignette.
#' 
#' @param gaucheInput             Raw data analysed by gauche() 
#' @param gaucheOutput            Object of class ga produced by gauche()
...
#' @export
# @references gauche paper here!
#' @author Alex Murison \email{Alexander.Murison@@icr.ac.uk} and Christopher Wardell \email{Christopher.Wardell@@icr.ac.uk}
#' @seealso \code{\link{ga-class}}, \code{\link{ga}}, \code{\link{gaucheReport}}, \code{\link{gauche_simple_data}},
#' \code{\link{gauche_hidden_data}}, \code{\link{gauche_synth_data}},
#' \code{\link{gauche_synth_data_jittered}}, \code{\link{BYB1_G07_pruned}}
#' @examples
...
gaucheReport<-function(gaucheInput,gaucheOutput,outType="complete",yRange=c(-250,0),output_file_prefix="") {
  ...
  observation_matrix = gaucheInput
  #observation_matrix = gaucheInput[,2:ncol(gaucheInput)]
  
  ## Get number of mutations from gaucheOutput
  number_of_mutations=length(gaucheOutput@names)
  
  ## Get number of clones from gaucheOutput
  number_of_clones=gaucheOutput@min
  
  ## Get number of cases (e.g. timepoints) from gaucheOutput
  number_of_cases=gaucheOutput@max
  
  ## We can now work out contamination status.  This is the reverse of calculating numBits in gauche()
  if(((gaucheOutput@nBits-number_of_clones-number_of_mutations)/number_of_cases)==number_of_clones){
    contamination=0
  }else{
    contamination=1
  }
  ...
  top_number<-min(5,nrow(gaucheOutput@solution))
  for (number_of_answers in 1:top_number) {
    ...
    answers<-gaucheOutput@solution[number_of_answers,]
    ...
    if(outType %in% c("complete")){
      ...
      #write.table(gaucheOutput@best,paste0(output_file_prefix,number_of_clones,"clones.highest.score.solution.",number_of_answers,"of",top_number,".txt"),sep="\t", row.names=FALSE,col.names=FALSE)
    }
    ...
    if(outType %in% c("complete","heatmap")){
      ...
      heatmap.plus(as.matrix(observation_matrix),Colv=NA,col=heatmapColours,scale="none",Rowv=NULL,labRow=gaucheOutput@names,RowSideColors=clones,cexRow=0.9)
      if(outType=="complete"){ dev.off() }
    }  
